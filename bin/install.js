...
async function main() {
  ...
  if (fs.existsSync(CLAUDE_DIR)) {
    const answer = await question(
      `${targetDisplay} already exists. Overwrite? (y/N): `,
    );
  ...
  try {
    ...
    if (fs.existsSync(settingsTemplatePath)) {
      const template = JSON.parse(
        fs.readFileSync(settingsTemplatePath, "utf8"),
      );
      let settings = {};
      ...
      if (fs.existsSync(settingsPath)) {
        try {
          ...
          console.warn(
            "  Warning: existing settings.json is invalid JSON, creating new one",
          );
          settings = {};
        }
      }
    ...
    console.log(
      `Read ${targetDisplay}ARCHITECTURE.md for full documentation.\n`,
    );
  } catch (err) {
