...
3. `intermittent == true` AND `stack_trace == false` → Phase D (pure intermittent needs deep investigation; if user gave us a file, Phase C can use it as a hint instead)
4. `vague == true` AND **no `domain_*` signal matched** AND `intermittent == false` AND `browser_visual == false` → Vague Gate (A.7)
5. `likely_not_code == true` → Phase C **investigation-only mode**
...
### B.2 -- Apply Edit with git-stash safety net

3. `git stash push -u -m "phase-b-{slug}" -- {file}` (atomic backup — safe because B.0 verified file was clean)
4. Apply the minimal Edit (single Edit call, one line changed)
5. **Detect test command by file presence (in order):**
   ...
   - Else → **skip Phase B** (no test command detectable), `git stash pop`, fall through to Phase C
6. Run the detected test command
7. Run typecheck if applicable: `test_commands.typecheck` OR auto-detect (`npx tsc --noEmit` if `tsconfig.json` exists)

### B.3 -- Verify

- **Both pass:** `git stash drop` (discard backup). Write LITE diagnosis with `Severity: TRIVIAL`, `Fix Type: code`, `Outcome: fixed`, `Committed: no`, Status `DIAGNOSED`. Return `{slug}: TRIVIAL`.
- **Either fails:** `git stash pop` (restore original atomically). Fall through to Phase C normal mode.
...
- RED for predicted reason → root cause confirmed. Go to C.6 → write diagnosis.
- RED for different reason → strike +1, update hypothesis.
...
- Once identified → go directly to C.6 → write diagnosis with instructions.
